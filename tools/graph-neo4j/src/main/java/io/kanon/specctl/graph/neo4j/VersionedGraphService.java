...
public final class VersionedGraphService {
    ...
    public List<String> ingestStatements(String generatorRunId, CanonicalIr ir, CodebaseIr codebaseIr) {
        List<String> statements = new ArrayList<>(schemaStatements());
        statements.add("MERGE (r:GeneratorRun {run_id: '" + escape(generatorRunId) + "'})");
        statements.add(node("Service", ir.service().stableId(), generatorRunId, ir.service().name(),
                ir.service().canonicalPath()));
        for (CanonicalIr.BoundedContext boundedContext : ir.boundedContexts()) {
            statements.add(node("BoundedContext", boundedContext.stableId(), generatorRunId, boundedContext.name(),
                    boundedContext.canonicalPath()));
            statements.add(
                    rel("Service", ir.service().stableId(), "DECLARES", "BoundedContext", boundedContext.stableId(),
                            generatorRunId));
            for (CanonicalIr.Aggregate aggregate : boundedContext.aggregates()) {
                statements.add(node("Aggregate", aggregate.stableId(), generatorRunId, aggregate.name(),
                        aggregate.canonicalPath()));
                statements.add(
                        rel("BoundedContext", boundedContext.stableId(), "DECLARES", "Aggregate", aggregate.stableId(),
                                generatorRunId));
                for (CanonicalIr.Entity entity : aggregate.entities()) {
                    statements.add(
                            node("Entity", entity.stableId(), generatorRunId, entity.name(), entity.canonicalPath()));
                    statements.add(rel("Aggregate", aggregate.stableId(), "DECLARES", "Entity", entity.stableId(),
                            generatorRunId));
                    for (CanonicalIr.Field field : entity.fields()) {
                        statements.add(
                                node("Field", field.stableId(), generatorRunId, field.name(), field.canonicalPath()));
                        statements.add(rel("Entity", entity.stableId(), "HAS_FIELD", "Field", field.stableId(),
                                generatorRunId));
                    }
                }
                for (CanonicalIr.Command command : aggregate.commands()) {
                    statements.add(node("Command", command.stableId(), generatorRunId, command.name(),
                            command.canonicalPath()));
                    statements.add(rel("Aggregate", aggregate.stableId(), "DECLARES", "Command", command.stableId(),
                            generatorRunId));
                    for (CanonicalIr.Rule rule : command.rules()) {
                        statements.add(node("Rule", rule.stableId(), generatorRunId, rule.id(), rule.canonicalPath()));
                        statements.add(rel("Command", command.stableId(), "GUARDED_BY", "Rule", rule.stableId(),
                                generatorRunId));
                    }
                }
            }
        ...
        for (CodebaseIr.Type type : codebaseIr.types()) {
            statements.add(node("CodeType", type.id(), generatorRunId, type.qualifiedName(),
                    "/code/types/" + escape(type.qualifiedName())));
            for (CodebaseIr.Method method : type.methods()) {
                statements.add(node("CodeMethod", method.id(), generatorRunId, method.name(),
                        "/code/methods/" + escape(method.id())));
                statements.add(rel("CodeType", type.id(), "DECLARES", "CodeMethod", method.id(), generatorRunId));
            }
        }
        for (CodebaseIr.Endpoint endpoint : codebaseIr.endpoints()) {
            statements.add(node("CodeEndpoint", endpoint.id(), generatorRunId,
                    endpoint.httpMethod() + " " + endpoint.fullPath(), endpoint.fullPath()));
            String ownerTypeId = endpoint.methodId().contains("#") ?
                    endpoint.methodId().substring(0, endpoint.methodId().indexOf('#')) : endpoint.methodId();
            statements.add(rel("CodeType", ownerTypeId, "EXPOSES", "CodeEndpoint", endpoint.id(), generatorRunId));
        }
        for (CodebaseIr.JpaEntity entity : codebaseIr.jpaEntities()) {
            statements.add(node("CodeJpaEntity", entity.id(), generatorRunId, entity.typeId(),
                    entity.tableName() == null ? "" : entity.tableName()));
        }
        for (CodebaseIr.Bean bean : codebaseIr.beans()) {
            statements.add(node("CodeBean", bean.id(), generatorRunId, bean.name(), bean.typeId()));
        }
        for (io.kanon.specctl.extraction.ir.Provenance anchor : codebaseIr.provenance()) {
            String nodeId = anchor.subjectId() + "#" + anchor.file() + ":" + anchor.startLine();
            statements.add(
                    "MERGE (a:CodeAnchor {node_id: '" + escape(nodeId) + "', version: '" + escape(generatorRunId) +
                            "'}) "
                            + "SET a.path = '" + escape(anchor.subjectId()) + "', a.file = '" + escape(anchor.file()) +
                            "', a.symbol = '"
                            + escape(anchor.symbol()) + "', a.startLine = " + anchor.startLine() + ", a.endLine = " +
                            anchor.endLine()
            );
        }
        return statements;
    ...
    public void ingest(String uri, String username, String password, String generatorRunId, CanonicalIr ir,
                       CodebaseIr codebaseIr) {
        try (Driver driver = GraphDatabase.driver(uri, AuthTokens.basic(username, password));
             org.neo4j.driver.Session session = driver.session()) {
            for (String statement : ingestStatements(generatorRunId, ir, codebaseIr)) {
                session.run(statement);
            }
        }
    }

    public List<String> queryContext(String uri, String username, String password, String runId,
    ...
    private String rel(String leftLabel, String leftId, String type, String rightLabel, String rightId,
                       String version) {
