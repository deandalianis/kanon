...
public final class MigrationService {
    ...
    private SpecDocument renameField(SpecDocument spec, MigrationDocument.Migration migration) {
        ...
        for (SpecDocument.BoundedContext context : spec.boundedContexts()) {
            ...
            for (SpecDocument.Aggregate aggregate : context.aggregates()) {
                if (!matches(migration.boundedContext(), context.name()) || !matches(migration.aggregate(), aggregate.name())) {
                ...
                for (SpecDocument.Entity entity : aggregate.entities()) {
                    ...
                    List<SpecDocument.Field> fields = entity.fields().stream()
                            .map(field -> field.name().equals(migration.from()) ? field.withName(migration.to()) : field)
                            .toList();
        ...
        List<SpecDocument.BoundedContext> contexts = spec.boundedContexts().stream()
                .map(context -> {
                    ...
                    List<SpecDocument.Aggregate> aggregates = context.aggregates().stream()
                            .map(aggregate -> aggregate.name().equals(migration.from()) ? aggregate.withName(migration.to()) : aggregate)
                            .toList();
                    ...
                    List<SpecDocument.Aggregate> aggregates = context.aggregates().stream()
                            .map(aggregate -> {
                                ...
                                List<SpecDocument.Command> commands = aggregate.commands().stream()
                                        .map(command -> command.name().equals(migration.from()) ? command.withName(migration.to()) : command)
                                        .toList();
                    ...
                    List<SpecDocument.Aggregate> aggregates = context.aggregates().stream()
                            .map(aggregate -> {
                                ...
                                List<SpecDocument.Event> events = aggregate.events().stream()
                                        .map(event -> event.name().equals(migration.from()) ? event.withName(migration.to()) : event)
                                        .toList();
                                List<SpecDocument.Command> commands = aggregate.commands().stream()
                                ...
                                List<SpecDocument.Command> commands = aggregate.commands().stream()
                                        .map(command -> {
                                            ...
                                            List<SpecDocument.Rule> rules = command.rules().stream()
                                                    .map(rule -> rule.withEnsure(rule.ensure().replace(migration.from(), migration.to())))
                                                    .toList();
    ...
    private boolean matches(String expected, String actual) {
        return expected == null || expected.isBlank() || expected.toLowerCase(Locale.ROOT).equals(actual.toLowerCase(Locale.ROOT));
    }
