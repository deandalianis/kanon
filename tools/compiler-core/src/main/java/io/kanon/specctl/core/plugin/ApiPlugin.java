        ...
        for (CanonicalIr.BoundedContext boundedContext : context.ir().boundedContexts()) {
            for (CanonicalIr.Aggregate aggregate : boundedContext.aggregates()) {
                ...
                source.append("@org.springframework.web.bind.annotation.RequestMapping(\"/")
                        .append(aggregate.canonicalName().replace('_', '-')).append("\")").append(System.lineSeparator());
                source.append("public class ").append(className).append(" {").append(System.lineSeparator());
                source.append("    private final ").append(runtimePackage).append(".").append(aggregate.name()).append("CommandHandler handler;").append(System.lineSeparator());
                source.append("    public ").append(className).append("(").append(runtimePackage).append(".").append(aggregate.name()).append("CommandHandler handler) {").append(System.lineSeparator());
                ...
                for (CanonicalIr.Command command : aggregate.commands()) {
                    Optional<String> securityExpression = PluginSupport.securityExpression(context.ir(), command.name());
                    securityExpression.ifPresent(expression -> source.append("    @org.springframework.security.access.prepost.PreAuthorize(\"")
                            .append(expression.replace("\"", "\\\""))
                            .append("\")").append(System.lineSeparator()));
                    source.append("    ").append(mapping(command.http().method())).append("(\"").append(command.http().path()).append("\")").append(System.lineSeparator());
                    source.append("    public org.springframework.http.ResponseEntity<Void> ").append(PluginSupport.lowerFirst(command.name())).append("(");
                    ...
                    source.append(") {").append(System.lineSeparator());
                    if (!command.inputFields().isEmpty()) {
                        source.append("        handler.").append(PluginSupport.lowerFirst(command.name())).append("(request);").append(System.lineSeparator());
                    } else {
                        source.append("        handler.").append(PluginSupport.lowerFirst(command.name())).append("(new ")
                                .append(domainPackage).append(".").append(command.name()).append("Request(java.util.Map.of()));").append(System.lineSeparator());
                    }
                    source.append("        return org.springframework.http.ResponseEntity.noContent().build();").append(System.lineSeparator());
                    source.append("    }").append(System.lineSeparator());
                }
                source.append("}").append(System.lineSeparator());
                context.writeFile("src/generated/java/api", packageBase.replace('.', '/') + "/" + className + ".java", source.toString());
            }
        }
    }
