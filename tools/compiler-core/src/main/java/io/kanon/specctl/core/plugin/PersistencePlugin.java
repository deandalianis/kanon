    ...
    public void apply(GenerationContext context) {
        ...
        for (CanonicalIr.BoundedContext boundedContext : context.ir().boundedContexts()) {
            for (CanonicalIr.Aggregate aggregate : boundedContext.aggregates()) {
                ...
                source.append("@jakarta.persistence.Table(name = \"").append(entity.table()).append("\")").append(System.lineSeparator());
                ...
                for (CanonicalIr.Field field : entity.fields()) {
                    ...
                    source.append("    private ").append(PluginSupport.mapType(field.type())).append(" ").append(field.name()).append(";").append(System.lineSeparator());
                }
                source.append("}").append(System.lineSeparator());
                context.writeFile("src/generated/java/persistence", packageBase.replace('.', '/') + "/" + entityName + ".java", source.toString());

                String repositorySource = PluginSupport.provenanceHeader(context)
                        + "package " + packageBase + ";" + System.lineSeparator()
                        + "public interface " + aggregate.name() + "Repository extends org.springframework.data.jpa.repository.JpaRepository<"
                        + entityName + ", java.util.UUID> {}" + System.lineSeparator();
                context.writeFile("src/generated/java/persistence", packageBase.replace('.', '/') + "/" + aggregate.name() + "Repository.java", repositorySource);

                String columns = entity.fields().stream()
                        .map(field -> "  " + field.name() + " " + PluginSupport.sqlType(field.type()) + (field.pk() ? " primary key" : ""))
                        .collect(Collectors.joining("," + System.lineSeparator()));
                String migration = "create table if not exists " + entity.table() + " (" + System.lineSeparator() + columns + System.lineSeparator() + ");" + System.lineSeparator();
                context.writeFile("src/generated/resources/db/migration", "V" + migrationVersion++ + "__" + entity.table() + ".sql", migration);
            }
        }
    }
}
