...
public final class TestPlugin implements CompilerPlugin {
    ...
    private void generateJUnitTest(GenerationContext context, CanonicalIr.Aggregate aggregate,
        ...
        source.append("    private ").append(domainPackage).append(".").append(aggregate.name()).append("Hooks hooks;").append(System.lineSeparator());
        source.append("    @Mock").append(System.lineSeparator());
        source.append("    private org.springframework.context.ApplicationEventPublisher eventPublisher;").append(System.lineSeparator());
        source.append("    @InjectMocks").append(System.lineSeparator());
        source.append("    private ").append(runtimePackage).append(".").append(aggregate.name()).append("CommandHandler handler;").append(System.lineSeparator());
        ...
        for (CanonicalIr.Command command : aggregate.commands()) {
            ...
            for (CanonicalIr.BddScenario scenario : command.scenarios()) {
                String methodName = "test_" + toSafeIdentifier(command.name()) + "_" + toSafeIdentifier(scenario.name());
                source.append(System.lineSeparator());
                ...
                if (expectsThrow) {
                    source.append("        assertThatThrownBy(() -> handler.").append(PluginSupport.lowerFirst(command.name()))
                            .append("(new ").append(requestType).append("(java.util.Map.of())))").append(System.lineSeparator());
                    for (CanonicalIr.BddStep then : scenario.then()) {
                        if (then.impl() != null && "throw".equals(then.impl().type())) {
                            ...
                            source.append("                .isInstanceOf(").append(exClass).append(".class);").append(System.lineSeparator());
                        }
                    }
                } else {
        ...
        source.append("}").append(System.lineSeparator());
        context.writeFile("src/generated/test/java", packageBase.replace('.', '/') + "/" + className + ".java", source.toString());
    }
    ...
    private void appendTestAssert(StringBuilder source, CanonicalIr.BddStep step) {
        ...
        switch (step.impl().type()) {
            case "emit" -> {
                ...
                source.append("        verify(eventPublisher).publishEvent(any(").append(event).append(".class));").append(System.lineSeparator());
            }
            case "call" -> {
                if (step.impl().service() != null && step.impl().method() != null) {
                    String args = step.impl().args() == null ? "" : step.impl().args().stream()
                    ...
                    source.append("        verify(").append(step.impl().service()).append(")")
                            .append(".").append(step.impl().method()).append("(").append(args).append(");").append(System.lineSeparator());
                }
            }
            case "set" -> {
        ...
        for (CanonicalIr.Command command : aggregate.commands()) {
            ...
            for (CanonicalIr.BddScenario scenario : command.scenarios()) {
                ...
                for (int i = 0; i < scenario.when().size(); i++) {
                    ...
                    feature.append("    ").append(keyword).append(scenario.when().get(i).step()).append(System.lineSeparator());
                }
                for (int i = 0; i < scenario.then().size(); i++) {
                    String keyword = i == 0 ? "Then " : "And ";
                    feature.append("    ").append(keyword).append(scenario.then().get(i).step()).append(System.lineSeparator());
                }
                feature.append(System.lineSeparator());
            }
        }
