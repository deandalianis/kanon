    ...
    public void apply(GenerationContext context) {
        ...
        for (CanonicalIr.BoundedContext boundedContext : context.ir().boundedContexts()) {
            for (CanonicalIr.Aggregate aggregate : boundedContext.aggregates()) {
                ...
                context.writeFile("src/generated/java/domain", packageBase.replace('.', '/') + "/" + dtoName + ".java", dtoSource);

                String hookSource = PluginSupport.provenanceHeader(context)
                        + "package " + packageBase + ";" + System.lineSeparator()
                        + "public interface " + aggregate.name() + "Hooks {" + System.lineSeparator()
                        + "    default void beforeCreate(Create" + aggregate.name() + "Request request) {}" + System.lineSeparator()
                        + "    default void afterCreate(" + dtoName + " dto) {}" + System.lineSeparator()
                        + "}" + System.lineSeparator();
                context.writeFile("src/generated/java/domain", packageBase.replace('.', '/') + "/" + aggregate.name() + "Hooks.java", hookSource);

                String createRequest = PluginSupport.provenanceHeader(context)
                        + "package " + packageBase + ";" + System.lineSeparator()
                        + "public record Create" + aggregate.name() + "Request(java.util.Map<String, Object> payload) {}" + System.lineSeparator();
                context.writeFile("src/generated/java/domain", packageBase.replace('.', '/') + "/Create" + aggregate.name() + "Request.java", createRequest);
                ...
                for (CanonicalIr.Command command : aggregate.commands()) {
                    ...
                    context.writeFile("src/generated/java/domain", packageBase.replace('.', '/') + "/" + requestName + ".java", requestSource);
                }
                ...
                if (aggregate.stateMachine() != null) {
                    String enumSource = PluginSupport.provenanceHeader(context)
                            ...
                            + "}" + System.lineSeparator();
                    context.writeFile("src/generated/java/domain", packageBase.replace('.', '/') + "/" + aggregate.stateMachine().name() + ".java", enumSource);
                }
            }
        }
    }
