...
public final class GenerationContext {
    ...
    public void writeFile(String ownedRoot, String relativePath, String content) {
        if (!manifest.ownsOutputRoots().contains(ownedRoot)) {
            diagnostics.error("PLUGIN_ROOT_VIOLATION", "Plugin attempted to write outside owned roots: " + ownedRoot, "/" + manifest.name());
            return;
        }
        if (!ownedRoot.startsWith("src/generated/")) {
            diagnostics.error("PLUGIN_ROOT_INVALID", "Generated output root must stay under src/generated/**", "/" + manifest.name());
            return;
        }
        ...
        if (fullPath.toString().contains("src\\main") || fullPath.toString().contains("src/main")) {
            diagnostics.error("PLUGIN_MAIN_MUTATION", "Generated code must never target src/main/**", "/" + manifest.name());
            return;
        }
