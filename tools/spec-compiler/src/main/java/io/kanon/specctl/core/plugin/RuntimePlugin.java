    ...
    public void apply(GenerationContext context) {
        ...
        for (CanonicalIr.BoundedContext boundedContext : context.ir().boundedContexts()) {
            for (CanonicalIr.Aggregate aggregate : boundedContext.aggregates()) {
                ...
                source.append("    private final ").append(domainPackage).append(".").append(aggregate.name())
                        .append("Hooks hooks;").append(System.lineSeparator());
                source.append("    private final org.springframework.context.ApplicationEventPublisher eventPublisher;")
                        .append(System.lineSeparator());
                source.append("    public ").append(className).append("(")
                        .append(domainPackage).append(".").append(aggregate.name()).append("Hooks hooks, ")
                        .append("org.springframework.context.ApplicationEventPublisher eventPublisher) {")
                        .append(System.lineSeparator());
                ...
                for (CanonicalIr.Command command : aggregate.commands()) {
                    ...
                    if (!command.scenarios().isEmpty()) {
                        ...
                        for (CanonicalIr.BddScenario scenario : command.scenarios()) {
                            source.append("        // Scenario: ").append(scenario.name())
                                    .append(System.lineSeparator());
                            for (CanonicalIr.BddStep given : scenario.given()) {
                                ...
                                if (given.impl() == null && given.sourceHint() == null) {
                                    stubMethods.add(stepResolver.stubMethod(given.step(), command.name(), stepIndex,
                                            requestType, packageBase));
                                }
                                stepIndex++;
                            }
                            for (CanonicalIr.BddStep then : scenario.then()) {
                                ...
                                if (then.impl() == null && then.sourceHint() == null) {
                                    stubMethods.add(
                                            stepResolver.stubMethod(then.step(), command.name(), stepIndex, requestType,
                                                    packageBase));
                                }
                                stepIndex++;
                            }
                        ...
                        source.append("        hooks.beforeCreate(new ").append(domainPackage).append(".Create")
                                .append(aggregate.name()).append("Request(java.util.Map.of()));")
                                .append(System.lineSeparator());
                        for (CanonicalIr.Rule rule : command.rules()) {
                            source.append("        // ").append(rule.type()).append(": ").append(rule.source())
                                    .append(System.lineSeparator());
                        }
                    }
                    source.append("    }").append(System.lineSeparator());
                ...
                source.append("}").append(System.lineSeparator());
                context.writeFile("src/generated/java/runtime",
                        packageBase.replace('.', '/') + "/" + className + ".java", source.toString());
            }
        }
    }
