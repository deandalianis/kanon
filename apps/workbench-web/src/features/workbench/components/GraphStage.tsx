...
import type { ExtractionResult, GraphEdge, GraphNode, GraphView } from "../../../types";
import { buildHotspots, type ExplorerArtifact, type ExplorerHotspot, type StructuralNodeType } from "../graphExplorerModel";
  ...
  return (
    <Panel className="graph-stage-panel">
      <SectionHeader
        ...
        badge={<StatusBadge tone={graphTone}>{graphSummary.nodes ? `${graphSummary.nodes} nodes` : "not loaded"}</StatusBadge>}
        actions={
          <button
            type="button"
      ...
      {!hasExtractionRun ? (
        ...
        <EmptyState
          ...
          detail="Open the graph stage after extraction to build the structural lineage for this workspace."
        />
      )}
    </Panel>
  );
...
function NodeInspector({
  ...
  return (
    <div className="graph-inspector-body">
      ...
      <KeyValueList
        items={[
          { label: "Bounded contexts", value: node.stats.boundedContextCount },
          { label: "Aggregates", value: node.stats.aggregateCount },
          { label: "Commands", value: node.stats.commandCount },
          { label: "Entities", value: node.stats.entityCount },
          { label: "Events", value: node.stats.eventCount }
  ...
  return (
    <div className="graph-inspector-section">
      ...
      {hotspots.length ? (
        ...
        <EmptyState title={`No ${title.toLowerCase()}`} detail="Nothing in the current graph matches this hotspot category." />
      )}
    </div>
  );
}
...
function buildGraphChips(node: GraphNode) {
  ...
  if (node.type === "service") {
    chips.push(`${node.stats.boundedContextCount} contexts`);
    chips.push(`${node.stats.aggregateCount} aggregates`);
  } else if (node.type === "bounded-context") {
    chips.push(`${node.stats.aggregateCount} aggregates`);
    chips.push(`${node.stats.commandCount} commands`);
  } else if (node.type === "aggregate") {
    chips.push(`${node.stats.commandCount} commands`);
    chips.push(`${node.stats.entityCount} entities`);
    chips.push(`${node.stats.eventCount} events`);
  } else if (node.type === "command") {
    chips.push(`${String(node.metadata.method ?? "").toUpperCase()} ${String(node.metadata.httpPath ?? "")}`.trim());
  } else if (node.type === "entity") {
    chips.push(`${String(node.metadata.fieldCount ?? 0)} fields`);
    chips.push(String(node.metadata.table ?? ""));
  } else if (node.type === "event") {
    chips.push(String(node.metadata.topic ?? ""));
  }
...
function getNodeVariant(type: string) {
  if (type === "service" || type === "bounded-context") {
    return "summary";
  }
...
function isExpandableNode(type: string) {
  return type === "service" || type === "bounded-context" || type === "aggregate";
}
