...
export function ProgressMeter({
    ...
    const boundedMax = max > 0 ? max : 1;
    const percent = Math.max(0, Math.min(100, (value / boundedMax) * 100));
    ...
    return (
        <div className="progress-meter">
            <div
                ...
                aria-valuenow={Math.round(Math.max(0, Math.min(boundedMax, value)) * 100) / 100}
            >
                <div
            ...
            {label ? <span className="progress-meter-label">{label}</span> : null}
        </div>
    );
}
